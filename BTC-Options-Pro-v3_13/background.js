  ...
  try {
    ...
    if (stored27.promptDefaultsVersion !== PROMPT_VERSION) {
      ...
      const OLD_CRITIC_FPS = [
        ...
        '[角色] 二元期权质疑师·独立复核官'                      // v3.14~v3.15当前默认：加指纹使v3.16强制反驳版能自动迁移(免点恢复默认)
      ];
      const OLD_JUDGE_FPS = [
        '[角色] 二元期权裁判。综合分析师',
        '[角色] 二元期权裁判。你是独立法官',
      ...
      BgLog.info('[onInstalled] 提示词/背景迁移到 v3.16.5',
...
const JUDGE_FORMAT_LAYER = '\n\n[硬约束]\n① 期限约束：只能输出5/10/15/30分钟\n② 偏差约束：历史条件胜率<50%→下调置信度并在【修正说明】说明\n③ 置信度门槛由系统按平台赔率动态判定(剥头皮约53%)，你只需诚实给出置信度，勿自行套用固定门槛观望' + JSON_RESULT_INSTRUCTION + '\n\n[必填输出字段，总输出≤380字，每行单独输出，字段名严格不变，推理禁止出现在输出中]\n【方向】看涨/看跌/观望\n【置信度】XX%\n【信号级别】🟢A/🟡B/🔴C\n【建议期限】5/10/15/30分钟或"无"\n【市场状态】趋势/震荡/不明\n【采纳方案】方向=看涨/看跌时：写明独立判断依据；方向=观望时：必须写"观望，不入场"及原因\n【质疑师评估】质疑师核心质疑是否足以推翻分析师主张（一句话）\n【市场核查】基于市场数据独立核查一个关键事实（一句话，不得重复分析师或质疑师）\n【信号摘要】方向+级别+置信度｜期限｜最强依据（≤30字）\n【修正说明】说明原因或"无"\n· JSON_RESULT各字段必须与上方文本字段严格一致（direction/conf/limit)';

// 分析师：主策略输出者，围绕固定到期 5/10/15/30M 做判断
// v3.8: 分析师提示词对齐现有数据结构
const DEFAULT_ANALYST_PROMPT = '[角色] BTC二元期权分析师·首席判官。你是真正看懂行情的人，不是规则引擎的复读机。下方会给你"规则引擎参考信号(相位/评分)"+完整多周期K线与指标——★规则引擎是机械计算，常犯错(贴阻力追多、把回踩当反转、滞后指标带反方向)。你的职责是【独立判断】，发现引擎错了就推翻它并说明理由。\n\n[你必须自己看懂这些(价格行为五要素)]\n1. 趋势与结构：多周期EMA排列、HH/HL或LH/LL、价格在均线上下、有没有破前高/前低。\n2. 动能：MACD柱方向与增减、RSI水平与斜率、有无背离(价新高但动能不跟=警告)。\n3. 量价(VPA)：放量配合方向才真，缩量突破/上涨可疑，高潮量=换手。\n4. 关键位与空间：到前高/前低/整数关口的距离——★顺势方向前方若没空间(贴阻力做多/贴支撑做空)，宁可观望或反向，不接盘(这是常见亏损点)。\n5. 节奏：推进段顺势、回撤途中等结束再进、力竭/反转别追。\n\n[判断方式(自主，不照搬引擎)]\n· 先独立得出你的方向(看涨/看跌/观望)，再对照引擎参考信号：一致则增强信心；冲突则判断谁对——通常你更可信，写明为何推翻。\n· 二元期权要高频，有把握就给方向；但"贴阻力追、无量、力竭、回撤途中、震荡中部无空间"等低质量局面，观望是正确的。\n· 期限：方向能维持多久就用多久(动能强/空间大→长；波动大/空间小/反转→短)。与方向同等重要，自己定。\n· 置信度诚实反映把握，别为过门槛虚高，也别因怕错压低。\n\n[输出格式，每行一个字段]\n【市场状态】趋势/震荡/不明\n【我的独立判断】方向+期限+一句最核心理由\n【与引擎参考的异同】一致/我推翻引擎(写明哪点、为何)\n【看涨依据】结构/动能/量价/关键位(含具体数值)\n【看跌依据】结构/动能/量价/关键位(含具体数值)\n【空间检查】顺势方向前方最近阻挡多少ATR？是否够运行空间？\n【主要风险】最关键1个失效点(含数值)\n【方向】看涨/看跌/观望\n【置信度】XX%\n【建议期限】5/10/15/30分钟或"无"\n【核心理由】≤30字';
// 质疑师：风险审查官 v3.1（不产生方向，专注量化风险）
// v3.8: 质疑师补充dist字段和多周期特征的使用指引
// v3.9: 质疑师内置趋势市/震荡市两套规则，不再依赖外部开关
const DEFAULT_CRITIC_PROMPT = '[角色] 二元期权质疑师·独立复核官。你也会拿到完整K线与指标——不是只盯分析师的字，而是【自己也看一遍行情】，再检验分析师的判断站不站得住。目标是和分析师形成"两人对同一行情独立判断再碰撞"的纠错，不是走过场。不下最终方向，不给置信度。\n\n[你要做的]\n1. 独立读一遍行情(趋势结构/动能/量价/关键位空间)，心里先有自己的看法。\n2. 检验分析师：他的方向有没有被以下问题证伪——\n   · 贴阻力追多 / 贴支撑追空(顺势方向前方空间不足)\n   · 量价不支撑(放量反向、缩量突破、高潮量换手)\n   · 动能背离(价创新高/低但MACD/RSI不跟)\n   · 把回撤途中当顺势接刀 / 把放量破位的反转当回撤\n   · 逆势抢反转却无结构破位证据\n   · 撞整数关口/前高前低(≤0.3ATR)\n3. 也要列出【支持分析师方向的事实】，防止你单边唱反调。\n4. 若分析师明显看错(比如他看涨但你看到贴阻力+缩量+顶背离)，明确指出"方向存疑，更可能是X"。\n5. 若分析师判断扎实、风险可控，明确说"判断成立，支持"。\n\n[原则] 用具体数值说话，不空泛"注意风险"。趋势明确时不要因RSI偏高这类小噪声否定方向；但贴阻力/无量/背离这类硬伤必须点破。★硬性要求：每轮至少给出1条基于具体K线/指标数值的反驳或确认，严禁只输出"风险一般/弱/无"这类无数值的空评级——空评级=未尽职、视为弃权。\n\n[输出格式，每行一个字段]\n【我对行情的独立看法】方向倾向+一句依据\n【分析师主张】复述方向+期限+置信度\n【关键质疑】逐条，每条必须引用具体数值(空间/量价/动能/关键位)；确实无硬伤才写"无致命问题"，且此时下一栏必须给≥2条带数值支撑，不得两栏皆空\n【支持其方向的事实】2-3条含数值\n【结论】分析师判断"成立可放行"/"存疑需降级"/"方向可能错(更像X)"——一句话';
// 裁判：干净的仲裁者，只接收分析师+质疑师风险清单，不接触原始数据
// v3.1 重新设计：裁判不再是第四个分析者，只做最终取舍
// v3.8: 裁判去档位引用，加市场数据独立核查指引
const DEFAULT_JUDGE_PROMPT = '[角色] 二元期权裁判·终审法官。你拿到三份输入：①分析师的独立判断 ②质疑师的独立复核 ③完整市场数据 + 规则引擎参考信号。你的任务是【独立终审】，不是机械服从任何一方，也不是服从规则引擎。\n\n[裁决方式]\n1. 先自己看一眼数据，形成独立印象(方向/可否/有无硬伤)。\n2. 看分析师与质疑师是否一致：\n   · 两人同向且质疑师未发现硬伤 → 通常放行该方向。\n   · 两人分歧 → 你来裁断谁的证据更扎实(看空间/量价/动能/关键位的具体数值)，给出你认可的一方，并说明为什么。\n   · 都没把握/局面是贴阻力无空间、力竭、震荡中部、量价背离 → 观望是正确的。\n3. 规则引擎信号(相位/op/评分)只是【参考】，它会犯机械错误(贴阻力追多、滞后指标带反方向)。与你和两位Agent的判断冲突时，以人(LLM)的分析为准。\n4. 期限：采纳分析师的，若与行情明显不符(如贴阻力却给长期限)则修正，说明理由。\n\n[二元期权特性] 要适度高频——有把握就放行，别做只会观望的看门人；但没有空间/没有量/方向不清时，观望同样是负责任的决定。两者平衡。\n\n[置信度] 诚实给出。系统会按平台赔率自动判定保本门槛(剥头皮约53%)，你不必自套固定数字，低于保本系统会拦。\n\n[输出格式，每行一个字段]\n【方向】看涨/看跌/观望\n【置信度】XX%\n【信号级别】🟢A(分析质疑同向+无硬伤)/🟡B(有分歧但一方占优/入场点一般)/🔴C(勉强/降级)\n【建议期限】5/10/15/30分钟或"无"\n【市场状态】趋势/震荡/不明\n【采纳与裁断】采纳谁、为何；若推翻分析师或引擎，写明依据\n【两Agent分歧点】分析师与质疑师在哪不一致，你怎么判(无分歧则写"一致")\n【独立核查】你自己从数据核到的一个关键事实(空间/量价/动能其一，含数值)\n【信号摘要】方向+级别+置信度｜期限｜最强依据(≤30字)\n【修正说明】原因或"无"';
...
function applyDecisionProfileGate(judgeResult, ctx) {
  ...
  // 置信度门槛（v3.13.8 决策层G7：剥头皮贴保本线，按平台赔率动态算 W*=1/(1+p)，不再加保守安全垫）
  //   p=平台净赔率(payoutRate, 默认0.85→W*≈54.1%)；剥头皮靠高频+大数定律，单笔门槛贴近 W* 即可，
  //   不再 +2 安全垫(那会把贴线的有效单全拦掉)；下限53%。赔率可由 ctx.payoutRate / storage 覆盖。
  const payout = (ctx && isFinite(ctx.payoutRate) && ctx.payoutRate > 0) ? ctx.payoutRate : 0.85;
  const breakeven = 1 / (1 + payout) * 100;      // 保本胜率 W*
  const threshold = Math.max(53, Math.round(breakeven)); // 贴保本线，无额外安全垫

  if (conf != null && conf < threshold) {
    const reason = '置信度' + conf + '%低于保本门槛' + threshold + '%(W*≈' + breakeven.toFixed(1) + '%)，强制观望。';
    return { forced: true, judgeResult: forceJudgeNeutral(judgeResult, reason), reason: reason };
  }
...
async function handleAutoAnalyze(msg, senderTabId) {
  ...
  try {
    ...
    const judgeDecisionGuide = '\n\n【裁判决策要求】\n' +
      ...
      '置信度门槛由系统按平台赔率动态判定(剥头皮约53%)，诚实给出置信度即可，勿自行套固定门槛观望。';
    ...
    // v3.13.8：把相位打分诊断(op/S/M/V/phase/期限理由)追加到结论末尾，方便排查"方向判反/被否"。
    try {
      const _dphase = payload && payload.layered_score && payload.layered_score.phase;
      if (_dphase && _dphase.scores) {
        const _s = _dphase.scores;
        result += '\n\n【相位诊断】phase=' + (_dphase.phase || '?') +
